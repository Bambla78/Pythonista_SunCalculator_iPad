...
def item_edited(sender):
    ...
    if len(removed) == 1 and removed[0] != ' My Location' and removed[0] != ' Choose location on map':
        item = removed[0]
...
def item_selected(sender):
	...
	if filter_is_set == 1:
		...
		#print(myobject['locations'][row]['title'])
...
def button4_tapped(sender):
	global printmoonstats, sathoehe
	...
	if float(vsettings['sathoehe'].text) < 250.0:
		vsettings['sathoehe'].text = '250.0'
		dialogs.hud_alert('Min. map size of 250 m² restored.', 'error', 1.8)			
...
def button1_tapped(sender):
	...
	if 'row' in globals():
		...
		if myobject['locations'][row]['title'] != ' My Location' and myobject['locations'][row]['title'] != ' Choose location on map':
...
#Output section
	...
	if norise == 0:
		...
		print('Sunrise (Hour, Direction, Height')
		aktp = 'Sunrise (Hour, Direction, Height)'
		printtabp(aktp)
		sonne(ah,am)
		sonnea = bq
		x = str(bq)
...
def mainprogram():
	...
	if flag == 1:
		...
		if k == True:	
			...
			#webview.close()
			dialogs.hud_alert('1. Long-tap location - 2. Press Share button - 3. Close Google Maps', 'success', 5)
			webview.wait_modal()
...
if devtype == 'iPhone':
	dialogs.hud_alert('This app is for iPads only.', 'error', 1.8)	
	exit()
